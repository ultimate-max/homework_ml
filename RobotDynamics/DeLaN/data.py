...
官方 DeLaN 数据与训练环境工具（对齐 deep_lagrangian_networks.utils，无 JAX 依赖）。
  ...
  from RobotDynamics.DeLaN import load_dataset, init_env
"""
...
from .import_data import OPTIONAL_DECOMP_KEYS, TRAIN_KEYS, inspect_pickle_dict
...
def validate_pickle_raw(data: dict[str, Any]) -> tuple[int, bool]:
    ...
    if "labels" not in data or "qp" not in data:
        raise KeyError("pickle 须含 labels、qp 等字段（见 RobotDynamics.delan_import）")
