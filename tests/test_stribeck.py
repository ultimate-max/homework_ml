...
def test_scv_opposes_motion() -> None:
    """τ_fri 与 qd 反号，与 robot pickle 中 τ−m−c−g 一致。"""
    qd = torch.tensor([[0.2, -0.3], [-0.15, 0.25]])
    k_v = torch.ones(2) * 0.05
    k_c = torch.ones(2) * 1.5
    k_a = torch.ones(2) * 10.0
    k_s = torch.ones(2) * 1.6
    v_s = torch.ones(2) * 0.05
    alpha = torch.ones(2) * 1.5
    tau = scv_torque(qd, k_v, k_c, k_a, k_s, v_s, alpha)
    assert torch.all(tau * qd < 0)
    assert torch.all(torch.sign(tau) == -torch.sign(qd))
